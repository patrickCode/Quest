        ...
        AdalModule.factory('ProtectedResourceInterceptor', ['adalAuthenticationService', '$q', '$rootScope', '$templateCache', function (authService, $q, $rootScope, $templateCache) {
            ...
            return {
                request: function (config) {
                    if (config) {
                        var unauthenticatedEndpoints = authService.getConfig().unauthenticatedEnpoints;
                        var isUnauthenticated = false;
                        angular.forEach(unauthenticatedEndpoints, function (endpoint) {
                            if (config.url == endpoint.url && config.method == endpoint.method)
